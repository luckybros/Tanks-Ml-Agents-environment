...
{
    public class TankShootingML : MonoBehaviour
    {
        ...
        // Added for RL Training
        [HideInInspector] public bool m_IsMLAgentControlled = false;
        [HideInInspector] public bool m_MLFireButtonDown = false;
        [HideInInspector] public bool m_MLFireButtonHeld = false;
        [HideInInspector] public bool m_MLFireButtonUp = false;
        [HideInInspector] public bool m_CanShoot => m_ShotCooldownTimer <= 0;
        ...
        private void Update ()
        {
            ...
            // Added function for ML
            if (m_IsMLAgentControlled)
            {
                MLAgentUpdate();
            }
            else if (!m_IsComputerControlled)
            {
                HumanUpdate();
            }
            else
        ...
        void MLAgentUpdate()
        {
            ...
            // Otherwise, if the fire button has just started being pressed...
            else if (m_ShotCooldownTimer <= 0 && m_MLFireButtonDown)
            {
                // ... reset the fired flag and reset the launch force.
                m_Fired = false;
                m_CurrentLaunchForce = m_MinLaunchForce;
            ...
            // Otherwise, if the fire button is being held and the shell hasn't been launched yet...
            else if (m_MLFireButtonHeld && !m_Fired)
            {
                // Increment the launch force and update the slider.
                m_CurrentLaunchForce += m_ChargeSpeed * Time.deltaTime;
            ...
            // Otherwise, if the fire button is released and the shell hasn't been launched yet...
            else if (m_MLFireButtonUp && !m_Fired)
            {
                // ... launch the shell.
                Fire ();
            }
                
            m_MLFireButtonDown = false;
            m_MLFireButtonUp = false;
        }
